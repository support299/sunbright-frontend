import { Clock, DollarSign, Download, Search, XCircle } from "lucide-react";
...
import { useGetCancelledProjectsQuery } from "../api/dashboardApi";
...
export default function CancellationsPage() {
  ...
  const { data: res, isLoading } = useGetCancelledProjectsQuery(filterParams);
  ...
  const reasons = useMemo(() => {
    const set = new Set(projects.map((p) => p.cancellation_reason).filter(Boolean));
    return Array.from(set).sort();
  }, [projects]);
  ...
  const filtered = useMemo(() => {
    ...
    return projects.filter((p) => {
      ...
      const matchReason = reasonFilter === "all" || p.cancellation_reason === reasonFilter;
      return matchSearch && matchTeam && matchReason;
    });
  }, [projects, search, teamFilter, reasonFilter]);

  const handleExport = () => {
    const headers = ["Name", "Rep", "Team", "Installer", "Lead Source", "Reason", "Contract", "Customer Since", "Clean Deal"];
    ...
    a.href = url;
    a.download = "cancelled-projects.csv";
    a.click();
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Cancelled Projects</h1>
  ...
  const totalValue = projects.reduce((sum, p) => sum + (Number(p.contract_amount) || 0), 0);
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <Card className="gap-0 border-border bg-card py-0">
          <CardContent className="p-5">
            <div className="flex items-start justify-between">
              <div className="space-y-2">
                <p className="text-sm font-medium text-muted-foreground">Contract value (sum)</p>
        ...
        <Card className="gap-0 border-border bg-card py-0">
          <CardContent className="p-5">
            <div className="flex items-start justify-between">
              <div className="space-y-2">
                <p className="text-sm font-medium text-muted-foreground">Data note</p>
                <p className="text-sm text-muted-foreground">Age / system size sync fields may be partial.</p>
              </div>
              <div className="rounded-lg bg-red-500/10 p-2.5">
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-xl border border-border bg-card p-4">
        ...
        <div>
          ...
          <select
            ...
            className="h-9 min-w-[180px] rounded-md border border-border bg-background px-3 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary"
          >
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  ...
                  <th className="px-3 py-3 text-left font-medium text-muted-foreground">Lead source</th>
              ...
              <tbody>
                {filtered.map((p) => (
                  <tr key={p.id} className="border-b border-border/50 transition-colors hover:bg-accent/30">
                    ...
                    <td className="whitespace-nowrap py-3 px-3 text-foreground">{p.lead_source || "—"}</td>
                    <td className="max-w-[200px] truncate py-3 px-3 text-foreground">{p.cancellation_reason || "—"}</td>
